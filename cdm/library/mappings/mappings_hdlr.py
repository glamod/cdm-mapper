...
def get_functions_module_path(imodel, log_level="INFO"):
    ...
    if not os.path.isfile(imodel_module):
        logger.error(f"No mapping functions module for model {imodel}")
        return
    else:
...
def load_code_tables_maps(imodel, codes_subset=None, log_level="INFO"):
    ...
    if not os.path.isdir(imodel_lib):
        logger.error(f"No model mapping library for model {imodel}")
        return
    ...
    if not os.path.isdir(imodel_codes_lib):
        logger.error(f"imodel code tables library path not found: {imodel_codes_lib}")
        return
...
def load_tables_maps(imodel, cdm_subset=None, log_level="INFO"):
    ...
    try:
        ...
        logger.error(f"Could not load mapping file {map_paths.get(key)}: {e}")
        return
    return maps
